      ...
      <div className="max-w-6xl mx-auto px-6 py-14">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-10">
          <div className="md:col-span-2">
            ...
            <p className="font-ibm text-sm text-white/50 leading-relaxed max-w-xs">
            ...
            <div className="mt-6 flex items-center gap-3">
              ...
              <div>
                <div className="font-ibm text-xs text-white/35 uppercase tracking-wider">Поддержка</div>
                <div className="font-ibm text-sm text-white/70 mt-0.5">support@prikazotmena.ru</div>
              </div>
            </div>
          </div>
          ...
          {cols.map(col => (
            <div key={col.title}>
              <div className="font-ibm text-xs font-semibold text-white/40 uppercase tracking-wider mb-4">{col.title}</div>
              <ul className="space-y-2.5">
                {col.links.map(link => (
                  <li key={link.id}>
                    <button
                      onClick={() => onNavigate(link.id)}
                      className="font-ibm text-sm text-white/55 hover:text-gold transition-colors"
                    >
                      {link.label}
                    </button>
                  </li>
        ...
        <div className="border-t border-white/10 mt-12 pt-6 flex flex-col md:flex-row items-center justify-between gap-3">
          <p className="font-ibm text-xs text-white/30">© 2024 ПриказОтмена. Все права защищены.</p>
