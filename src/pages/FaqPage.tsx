    ...
    <div className="pt-16 min-h-screen bg-background">
      <div className="bg-white border-b border-grey-200">
        <div className="max-w-4xl mx-auto px-6 py-8">
          ...
          <h1 className="font-cormorant font-bold text-grey-900 text-3xl md:text-4xl">Часто задаваемые вопросы</h1>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
        <div className="space-y-2">
          {faqs.map((faq, i) => (
            <div key={i} className="bg-white rounded-2xl shadow-card overflow-hidden">
              ...
              >
                <span className="font-ibm font-medium text-grey-900 text-sm leading-relaxed">{faq.q}</span>
              ...
              {open === i && (
                <div className="px-6 pb-6 animate-fade-in">
                  <div className="pl-0 border-t border-grey-200 pt-4">
                    <p className="font-ibm text-sm text-grey-500 leading-relaxed">{faq.a}</p>
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="mt-12 bg-blue rounded-3xl p-8 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <h3 className="font-cormorant font-bold text-white text-2xl mb-1.5">Не нашли ответ?</h3>
            <p className="font-ibm text-white/70 text-sm">Ответим в течение 2 рабочих часов</p>
          </div>
          <div className="flex gap-3">
            <button className="flex items-center gap-2 px-5 py-2.5 rounded-full border border-white/30 text-white font-ibm text-sm hover:border-white/60 transition-colors">
              <Icon name="MessageCircle" size={16} />Написать
            </button>
            <button className="flex items-center gap-2 px-5 py-2.5 rounded-full bg-gold text-grey-900 font-ibm font-semibold hover:bg-gold-light transition-colors">
